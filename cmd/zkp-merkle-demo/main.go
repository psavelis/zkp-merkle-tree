...
func runVerify(args []string) error {
	...
	hashName := fs.String("hash", "", "hash backend override for legacy proofs: sha256 or poseidon")
	...
	selectedHash := proof.HashName
	if *hashName != "" {
		if proof.HashName != "" && !strings.EqualFold(*hashName, proof.HashName) {
			return fmt.Errorf("proof hash %q does not match --hash %q", proof.HashName, *hashName)
		}
		selectedHash = *hashName
	}
	if selectedHash == "" {
		return errors.New("proof hash_name is missing; supply --hash for legacy proofs")
	}
	hasher, err := selectHasher(selectedHash)
	if err != nil {
		return err
	}
