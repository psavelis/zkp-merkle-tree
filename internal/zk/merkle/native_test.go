...
func TestNodeCommitmentRejectsNilInputs(t *testing.T) {
	_, err := NodeCommitment(nil, big.NewInt(1))
	if err == nil {
		t.Fatal("expected nil left input to fail")
	}

	_, err = NodeCommitment(big.NewInt(1), nil)
	if err == nil {
		t.Fatal("expected nil right input to fail")
	}
}
