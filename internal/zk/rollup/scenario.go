...
func BuildBalanceUpdateScenario(accountID int, oldBalance int, amount int, nonce int) (BalanceUpdateScenario, error) {
	...
	if accountID < 0 || oldBalance < 0 || nonce < 0 {
		return BalanceUpdateScenario{}, fmt.Errorf("account id, old balance, and nonce must be non-negative")
	}
	newBalance, err := zkints.CheckedAdd(oldBalance, amount)
	if err != nil {
		return BalanceUpdateScenario{}, fmt.Errorf("compute new balance: %w", err)
	}
